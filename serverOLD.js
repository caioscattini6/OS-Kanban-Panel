// server.js com validação de status e integração ContaAzul (Puppeteer)
...
app.post('/api/os', (req, res) => {
  ...
  ordemServico.push({ numero, status, urgente: false, observacoes: "", criadoEm: new Date().toISOString(), history: [{ status, at: new Date().toISOString() }] });
...
app.put('/api/os/:numero', (req, res) => {
  ...
  // registrar mudança de status em history
  if (status && status !== ordemServico[index].status) {
    ordemServico[index].status = status;
    ordemServico[index].history = ordemServico[index].history || [];
    ordemServico[index].history.push({ status, at: new Date().toISOString() });
  }
...
/* ------------------ NOVA ROTA: integração ContaAzul (Puppeteer) ------------------ */
/* Endpoint: POST /api/search
   Body: { numero: '12345' }
   Aciona o controller que abre Chrome (non-headless), navega até ContaAzul e executa a busca.
*/
...
app.post('/api/search', async (req, res) => {
  ...
  if (!numero) return res.status(400).json({ ok: false, error: 'numero is required' });

  try {
    // Execute the automation
    const result = await puppeteerController.searchOS(numero);
    if (result.ok) return res.json({ ok: true, message: result.message });
    else return res.status(500).json({ ok: false, error: result.error });
  } catch (err) {
